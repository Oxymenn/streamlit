...
def app():
    ...
    if st.button("Générer et publier l'article"):
        if not prompt or not keyword:
            ...
            try:
                with st.spinner("Génération de l'article en cours..."):
                    article_content = generate_article(prompt, keyword)
                ...
                if st.button("Publier l'article généré sur tous les sites"):
                    results = []
                    with concurrent.futures.ThreadPoolExecutor() as executor:
                        future_to_site = {executor.submit(publish_post_rest, get_site_config(site["key"]), f"Article sur {keyword}", article_content): site["name"] for site in sites if get_site_config(site["key"])}
