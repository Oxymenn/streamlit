...
def app():
    ...
    if uploaded_file:
        ...
        main_sheet = st.selectbox("Choisissez la feuille contenant les données principales", sheets, key='main_sheet')
        secondary_sheet = st.selectbox("Choisissez la feuille contenant les données secondaires", sheets, key='secondary_sheet')
        ...
        with col1:
            url_start_column = st.selectbox("Sélectionnez la colonne des URL de départ", df_main.columns, key='url_start_column')
        with col2:
            url_end_column = st.selectbox("Sélectionnez la colonne des URL de destination", df_main.columns, key='url_end_column')
        
        col3, col4 = st.columns(2)
        with col3:
            embedding_column = st.selectbox("Sélectionnez la colonne des Embeddings", df_main.columns, key='embedding_column')
        with col4:
            anchor_column = st.selectbox("Sélectionnez la colonne des ancres de liens", df_main.columns, key='anchor_column')
    ...
    if 'metrics_df' in st.session_state:
        ...
        with col5:
            url_start_filter = st.selectbox("Sélectionnez des URLs de départ :", options=["Choose an option"] + list(df[url_start_column].unique()), key='url_start_filter')
        with col6:
            url_end_filter = st.selectbox("Sélectionnez des URLs de destination :", options=["Choose an option"] + list(df[url_end_column].unique()), key='url_end_filter')
